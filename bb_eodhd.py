...
def api(path, token, params=None):
    ...
    for attempt in range(4):
        try:
            ...
            if not isinstance(obj,list): raise DataError('Unexpected provider payload')
            return obj
...
def catalog(token):
    records=[]
    for exchange in ('INDX','FOREX','COMM'):
        try:
            ...
            if 'access denied' in str(exc): raise
...
def validate(df, cfg):
    ...
    if df.index.min()>pd.Timestamp(cfg['start'])+pd.Timedelta(days=10): raise DataError('Truncated requested history')
    if df.close.pct_change().abs().gt(.35).any(): raise DataError('Structural move >35%: manual review required')
...
def build(token):
    ...
    links=''.join('<tr data-region="'+html.escape(m['region'],quote=True)+'"><td>'+html.escape(m['region'])+'</td><td><a href="'+m['id']+'.html">'+html.escape(m['name'])+'</a></td><td>'+m['last_date']+'</td><td>'+str(round(m['total_return']*100,2))+'</td></tr>' for m in summaries)
    body='<p>By Murat KONUKLAR · EODHD only · Generated '+stamp+'</p><p id="age"></p><p>Validated '+str(len(summaries))+'/'+str(len(assets))+' instruments. Partial coverage is explicitly disclosed. No proxy or synthetic market data.</p>'
